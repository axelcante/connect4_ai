...
class Info(tk.Frame):
    ...
    def __init__(self, master=None):
        ...
        police = font.Font(family="Arial",size=36,weight="bold") 
        self.t = tk.Label(self, text="Connect4 AI", font=police, bg ="white")
        self.t.grid(sticky=tk.NSEW, pady=20)
...
if __name__ == "__main__":
    ...
    tk.Button(root, text="Try again (?)", command=restart).grid(row=3, column=0, pady=5)
    tk.Button(root, text="Step back", command=step_back).grid(row=2, column=0, pady=2)
    tk.Button(root, text = "Exit", command = close).grid(row=4,column = 0, pady = 2)
